...
/**
 * Head+tail truncation: keeps the start AND end of output so tail-critical
 * information (footers, continuation hints, last results) is not silently
 * discarded. This mitigates the "lost-in-the-middle" degradation documented
 * in the LLM output degradation research.
 */
export function formatToolText(text: string, options: ToolOutputOptions = {}): string {
  ...
  if (budget <= 0) return text.slice(0, maxOutputChars);
...
/**
 * One-line stamp identifying which client a result came from.
 * Prevents the model from blending stale results across clients (context poisoning).
 */
...
export async function relayToolToApi(
  ...
  try {
      ...
      while (Date.now() < deadline) {
        ...
        if (job.status === "failed") {
          return toolTextResponse(
            `Failed: ${(job.error as string) ?? "Unknown error"}`,
            outputOptions,
            true
          );
        }
    ...
    return toolTextResponse(
      `Failed to relay to primary: ${(err as Error).message || err}`,
      outputOptions,
      true
    );
  }
...
export interface SendAndWaitOptions {
  ...
  /** When true, prepend a client identity stamp to successful output. */
  stampClient?: boolean;
}
...
export async function sendAndWait(options: SendAndWaitOptions): Promise<ToolTextResponse> {
  ...
  if (isFailure) {
    ...
    return toolTextResponse(
      text,
      { maxOutputChars: options.maxOutputChars, truncationHint: options.truncationHint },
      true
    );
  }
